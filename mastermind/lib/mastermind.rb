	...
	class HumanPlayer
		...
		def enter_guess
			while guesses.include?(nil)
				...
				guesses.map!.with_index { |guess, index| guess = get_players_guess(index)}
			end
		end

		def get_players_guess(index)
			loop do
				puts guesses.inspect
				guess = players_entered_guess(index)
				if valid?(guess)
					break
				else
					puts "nope"
		...
		def players_entered_guess(guess_number)
			print "Letter #{guess_number + 1}: "
			gets.chomp.upcase
		end

		def get_players_name    # gets the players name, this method is fine
			print "What is your name: "
			name = gets.chomp
		end

		#This method asks the user for thier guess and makes sure it is valid   #Failed, does too much!
		# def enter_guess
		# 	i = 0
		# 	while i < 4
		# 		print "Letter #{i + 1}: "
		# 		guesses[i] = gets.chomp.upcase
		# 		if LETTERS.include?(guesses[i])
		# 			i += 1
		# 		else
		#
		# 		end
		...
		# This method asks the player to enter a code for the computer to try and brake and it also makes sure the code is valid
		def enter_code      # Failed does too much and is basicly the same as the method above, these two can be refactored into one
			code = []
			i = 0
